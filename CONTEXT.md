...
| **Served mode** | Planned opt-in (`serve` cargo feature): `PGlite::open_served(dir, ServeOptions)` spawns the bundled `bin/postgres` as a child postmaster on a private unix socket (`listen_addresses=''` — no networking), pools N connections behind the same PGlite API. True multi-connection (real shm lock table, MVCC across sessions). Feature-gated: without `serve`, the API and code do not exist. Exempt from the one-boot constraint (no in-process engine boot). |
