...
- Phase 4 notes: live_query takes sql WITHOUT params in v1 (params in CREATE VIEW body are not valid Postgres; reference TS behavior unclear — deviation documented). Listener map upgraded to Vec<callback> per channel (shared base tables across live queries). Refresh runs on a dedicated per-query thread via futures::executor::block_on — notify dispatch only wakes it (re-entrancy risk from blueprint resolved). unsubscribe leaves triggers + listener entries in place (dead-flag neutralized) — minor leak, acceptable v1.
